<template>
  <div class="rst-activity">
    <div class="activity-txt" @click="actmodel = true">
      <span>满减</span>
      <span>{{ activities[0].description }}</span>
    </div>
    <div class="activity-count" @click="actmodel = true">
      <p>{{ activities.length }}个优惠 <i class="fa fa-caret-down"></i></p>
    </div>
    <!-- 优惠弹窗 -->
    <transition name="fade">
      <div class="act-model" v-show="actmodel">
        <div class="activity-sheet">
          <i class="fa fa-remove" @click="actmodel = false"></i>
          <h2>{{ activities.length }}个优惠</h2>
          <ul>
            <li v-for="(item, i) in activities" :key="i">
              <span style="background: #f07373">{{ item.icon_name }}</span>
